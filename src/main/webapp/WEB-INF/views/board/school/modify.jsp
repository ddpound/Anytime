...
${userNickname }
${userSchool }
<h1 style="color:skyblue;">${sectionName}게시판 글수정</h1><br>
		...
		<form action="${contextPath }/modify_save" method="post">
			...
			<div class="form-group" style="width:600px;">
				<c:if test="${tagList != null }">
					<input type="text" name="tag" class="form-control" value="${tagList }">
				</c:if>
				<c:if test="${tagList == null }">
			...
			<div class="form-group form-check">
    			...
    			<c:if test="${board.nickname == '익명이' }">
					<input type="checkbox" class="form-check-input" name="unknown" checked="checked">
				</c:if>
				<c:if test="${board.nickname != '익명이' }">
					<input type="checkbox" class="form-check-input" name="unknown">
				</c:if>
				   &nbsp;&nbsp;&nbsp;&nbsp;익명이
				</label>
			</div>
			<div class="w3-bar">
				<button type="button" class="w3-bar-item w3-button w3-border" onclick="location.href='${contextPath}/list/${board.section }'">
